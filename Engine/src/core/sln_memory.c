...
void *sln_allocate(u64 size, memory_tag tag)
{
    if (tag == MEMORY_TAG_UNKNOWN)
    {
        SLN_WARN("sln_allocate called using MEMORY_TAG_UNKNOWN. Re-class this allocation.");
    }
...
void sln_free(void *block, u64 size, memory_tag tag)
{
    if (tag == MEMORY_TAG_UNKNOWN)
    {
...
void *sln_zero_memory(void *block, u64 size)
{
    return platform_zero_memory(block, size);
}

void *sln_copy_memory(void *dest, const void *source, u64 size)
{
    return platform_copy_memory(dest, source, size);
}

void *sln_set_memory(void *dest, i32 value, u64 size)
{
