...
b8 application_create(game* game_inst) {
    if (game_inst->application_state) {
        SLN_ERROR("application_create called more than once.");
        return false;
    }

    game_inst->application_state = sln_allocate(sizeof(application_state), MEMORY_TAG_APPLICATION);
    ...
    if (!initialize_logging(&app_state->logging_system_memory_requirement, app_state->logging_system_state)) {
        SLN_ERROR("Failed to initialize logging system; shutting down.");
        return false;
    }
    ...
    if (!event_initialize()) {
        SLN_ERROR("Event system failed initialization. Application cannot continue.");
        return false;
    }
    ...
    if (!renderer_initialize(game_inst->app_config.name, &app_state->platform)) {
        SLN_FATAL("Failed to initialize renderer. Aborting application.");
        return false;
    }

    // Initialize the game.
    if (!app_state->game_inst->initialize(app_state->game_inst)) {
        SLN_FATAL("Game failed to initialize.");
        return false;
    }
    ...
    while (app_state->is_running) {
        ...
        if (!app_state->is_suspended) {
            ...
            if (!app_state->game_inst->update(app_state->game_inst, (f32)delta)) {
                SLN_FATAL("Game update failed, shutting down.");
                app_state->is_running = false;
                break;
            }
            ...
            if (!app_state->game_inst->render(app_state->game_inst, (f32)delta)) {
                SLN_FATAL("Game render failed, shutting down.");
                app_state->is_running = false;
                break;
            }
...
b8 application_on_resized(u16 code, void* sender, void* listener_inst, event_context context) {
    if (code == EVENT_CODE_RESIZED) {
        ...
        if (width != app_state->width || height != app_state->height) {
            ...
            SLN_DEBUG("Window resize: %i, %i", width, height);

            // Handle minimization
            if (width == 0 || height == 0) {
                SLN_INFO("Window minimized, suspending application.");
                app_state->is_suspended = true;
                return true;
            } else {
                if (app_state->is_suspended) {
                    SLN_INFO("Window restored, resuming application.");
                    app_state->is_suspended = false;
                }
