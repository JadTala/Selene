...
b8 vulkan_object_shader_create(vulkan_context *context, vulkan_object_shader *out_shader)
{
    ...
    for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; ++i)
    {
        ...
        {
            SLN_ERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_OBJECT);
            return false;
        }
    }
    ...
    sln_zero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
...
#define ATTRIBUTE_COUNT 2
    ...
    sln_zero_memory(stage_create_infos, sizeof(stage_create_infos));
    for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; ++i)
    {
    ...
    if (!vulkan_graphics_pipeline_create(
            ...
            &out_shader->pipeline))
    {
        SLN_ERROR("Failed to load graphics pipeline for object shader.");
        return false;
    }
    ...
    {
        SLN_ERROR("Vulkan buffer creation failed for object shader.");
        return false;
    }
    ...
    {
        SLN_ERROR("Material instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_object_shader_update_object(vulkan_context *context, struct vulkan_object_shader *shader, geometry_render_data data)
{
    ...
    sln_zero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    ...
    f32 s = (sln_sin(accumulator) + 1.0f) / 2.0f; // scale from -1, 1 to 0, 1
...
b8 vulkan_object_shader_acquire_resources(vulkan_context *context, struct vulkan_object_shader *shader, u32 *out_object_id)
{
    ...
    *out_object_id = shader->object_uniform_buffer_index;
    ...
    {
        SLN_ERROR("Error allocating descriptor sets in shader!");
        return false;
    }
...
void vulkan_object_shader_release_resources(vulkan_context *context, struct vulkan_object_shader *shader, u32 object_id)
{
    ...
    if (result != VK_SUCCESS)
    {
        SLN_ERROR("Error freeing object shader descriptor sets!");
    }
