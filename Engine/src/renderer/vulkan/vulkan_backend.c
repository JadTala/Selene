...
b8 vulkan_renderer_backend_initialize(renderer_backend *backend, const char *application_name)
{
    ...
    app_info.pEngineName = "Selene Engine";
    ...
    SLN_DEBUG("Required extensions:");
    u32 length = darray_length(required_extensions);
    for (u32 i = 0; i < length; ++i)
    {
        SLN_DEBUG(required_extensions[i]);
    }
#endif
...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
#if defined(_DEBUG)
    SLN_INFO("Validation layers enabled. Enumerating...");
    ...
    {
        SLN_INFO("Searching for layer: %s...", required_validation_layer_names[i]);
        b8 found = false;
        ...
        if (!found)
        {
            SLN_FATAL("Required validation layer is missing: %s", required_validation_layer_names[i]);
            return false;
        }
    }
    SLN_INFO("All required validation layers are present.");
#endif
    ...
    SLN_INFO("Vulkan Instance created.");

    // Debugger
#if defined(_DEBUG)
    SLN_DEBUG("Creating Vulkan debugger...");
    ...
    SLN_ASSERT_MSG(func, "Failed to create debug messenger!");
    VK_CHECK(func(context.instance, &debug_create_info, context.allocator, &context.debug_messenger));
    SLN_DEBUG("Vulkan debugger created.");
#endif

    // Surface
    SLN_DEBUG("Creating Vulkan surface...");
    if (!platform_create_vulkan_surface(&context))
    {
        SLN_ERROR("Failed to create platform surface!");
        return false;
    }
    SLN_DEBUG("Vulkan surface created.");

    // Device creation
    if (!vulkan_device_create(&context))
    {
        SLN_ERROR("Failed to create device!");
        return false;
    }
    ...
    {
        SLN_ERROR("Error loading built-in basic_lighting shader.");
        return false;
    }
    ...
    sln_zero_memory(verts, sizeof(vertex_3d) * vert_count);
    ...
    {
        SLN_ERROR("Failed to acquire shader resources.");
        return false;
    }

    // TODO: end temp code

    SLN_INFO("Vulkan renderer initialized successfully.");
    return true;
}
...
void vulkan_renderer_backend_shutdown(renderer_backend *backend)
{
    ...
    SLN_DEBUG("Destroying Vulkan device...");
    vulkan_device_destroy(&context);

    SLN_DEBUG("Destroying Vulkan surface...");
    if (context.surface)
    {
...
#if defined(_DEBUG)
    SLN_DEBUG("Destroying Vulkan debugger...");
...
#endif
    SLN_DEBUG("Destroying Vulkan instance...");
...
void vulkan_renderer_backend_on_resized(renderer_backend *backend, u16 width, u16 height)
{
    ...
    SLN_INFO("Vulkan renderer backend->resized: w/h/gen: %i/%i/%llu", width, height, context.framebuffer_size_generation);
}
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend *backend, f32 delta_time)
{
    ...
    if (context.recreating_swapchain)
    {
        ...
        {
            SLN_ERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
        SLN_INFO("Recreating swapchain, booting.");
        return false;
    }
    ...
    if (context.framebuffer_size_generation != context.framebuffer_size_last_generation)
    {
        ...
        if (!vulkan_result_is_success(result))
        {
            SLN_ERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (2) failed: '%s'", vulkan_result_string(result, true));
            return false;
        }
    ...
    if (!vulkan_fence_wait(
            ...
            UINT64_MAX))
    {
        SLN_WARN("In-flight fence wait failure!");
        return false;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend *backend, f32 delta_time)
{
    ...
    {
        SLN_ERROR("vkQueueSubmit failed with result: %s", vulkan_result_string(result, true));
        return false;
    }
    ...
    void *user_data)
{
    ...
    case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
        SLN_ERROR(callback_data->pMessage);
        break;
    case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
        SLN_WARN(callback_data->pMessage);
        break;
    case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
        SLN_INFO(callback_data->pMessage);
        break;
    case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
        SLN_TRACE(callback_data->pMessage);
        break;
    }
    return VK_FALSE;
}
...
i32 find_memory_index(u32 type_filter, u32 property_flags)
{
    ...
    SLN_WARN("Unable to find suitable memory type!");
    return -1;
}
...
void create_command_buffers(renderer_backend *backend)
{
    if (!context.graphics_command_buffers)
    {
        ...
        for (u32 i = 0; i < context.swapchain.image_count; ++i)
        {
            sln_zero_memory(&context.graphics_command_buffers[i], sizeof(vulkan_command_buffer));
        }
    }
    ...
    SLN_DEBUG("Vulkan command buffers created.");
}
...
b8 recreate_swapchain(renderer_backend *backend)
{
    ...
    if (context.recreating_swapchain)
    {
        SLN_DEBUG("recreate_swapchain called when already recreating. Booting.");
        return false;
    }
    ...
    {
        SLN_DEBUG("recreate_swapchain called when window is < 1 in a dimension. Booting.");
        return false;
    }
...
b8 create_buffers(vulkan_context *context)
{
    ...
    {
        SLN_ERROR("Error creating vertex buffer.");
        return false;
    }
...
void vulkan_renderer_create_texture(const char *name, b8 auto_release, i32 width, i32 height, i32 channel_count, const u8 *pixels, b8 has_transparency, texture *out_texture)
{
    ...
    out_texture->internal_data = (vulkan_texture_data *)sln_allocate(sizeof(vulkan_texture_data), MEMORY_TAG_TEXTURE);
    ...
    {
        SLN_ERROR("Error creating texture sampler: %s", vulkan_result_string(result, true));
        return;
    }
...
void vulkan_renderer_destroy_texture(struct texture *texture)
{
    ...
    sln_zero_memory(&data->image, sizeof(vulkan_image));
    vkDestroySampler(context.device.logical_device, data->sampler, context.allocator);
    data->sampler = 0;

    sln_free(texture->internal_data, sizeof(vulkan_texture_data), MEMORY_TAG_TEXTURE);
    sln_zero_memory(texture, sizeof(struct texture));
}