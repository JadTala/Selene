...
void linear_allocator_create(u64 total_size, void *memory, linear_allocator *out_allocator)
{
    if (out_allocator)
    {
        ...
        {
            out_allocator->memory = sln_allocate(total_size, MEMORY_TAG_LINEAR_ALLOCATOR);
        }
    }
}
void linear_allocator_destroy(linear_allocator *allocator)
{
    if (allocator)
    {
        ...
        {
            sln_free(allocator->memory, allocator->total_size, MEMORY_TAG_LINEAR_ALLOCATOR);
        }
        allocator->memory = 0;
...
void *linear_allocator_allocate(linear_allocator *allocator, u64 size)
{
    if (allocator && allocator->memory)
    {
        if (allocator->allocated + size > allocator->total_size)
        {
            ...
            SLN_ERROR("linear_allocator_allocate - Tried to allocate %lluB, only %lluB remaining.", size, remaining);
            return 0;
        }
    ...
    SLN_ERROR("linear_allocator_allocate - provided allocator not initialized.");
    return 0;
}
...
void linear_allocator_free_all(linear_allocator *allocator)
{
    ...
    {
        allocator->allocated = 0;
        sln_zero_memory(allocator->memory, allocator->total_size);
    }
}