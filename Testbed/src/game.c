...
void camera_pitch(game_state *state, f32 amount)
{
    ...
    state->camera_euler.x = SLN_CLAMP(state->camera_euler.x, -limit, limit);
...
{
    SLN_DEBUG("game_initialize() called!");
...
b8 game_update(game *game_inst, f32 delta_time)
{
    ...
    {
        SLN_DEBUG("Allocations: %llu (%llu this frame)", alloc_count, alloc_count - prev_alloc_count);
    }
